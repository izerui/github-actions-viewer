...
class PollingEngineTest {
    ...
    @Test
    fun `已完成的 run 只拉一次 jobs 之后不再重复请求`() =
        runTest {
    ...
    @Test
    fun `run 由运行中转为完成时应再拉一次 jobs 取终态`() =
        runTest {
    ...
    @Test
    fun `连续多轮拿不到仓库才认定为没有 GitHub remote`() =
        runTest {
            val transport = RecordingTransport({ runsJson("completed", "success") })
    ...
    @Test
    fun `配额耗尽时进入 RateLimited 并等到重置时刻`() =
        runTest {
    ...
    private fun loaded(engine: PollingEngine) = assertInstanceOf(ViewState.Loaded::class.java, engine.state.value)

    @Test
    fun `加载更多把历史记录追加到该 workflow 末尾`() =
        runTest {
            val transport =
                RecordingTransport(
